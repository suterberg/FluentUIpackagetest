...
from PySide6.QtCore import QDir, qInstallMessageHandler, QFile, QTextStream, QtMsgType, QStandardPaths, QDateTime, \
    qDebug, QSysInfo
import sys
import platform
import threading
from .helper import AppInfo
import os
...
def setup(argv: list, app: str, level: int=4):
    ...
    qDebug(f"[AppPath]  {argv[0]}")
    qDebug(f"[QtVersion]  {QtCore.__version__}")
    qDebug(f"[ProcessId] {os.getpid()}")
    qDebug(f"[GitHashCode] {AppInfo().commit_hash}")
    qDebug(f"[DeviceInfo]")
    qDebug(f"  [DeviceId] {QSysInfo.machineUniqueId()}")
    qDebug(f"  [Manufacturer] {platform.platform()}")
